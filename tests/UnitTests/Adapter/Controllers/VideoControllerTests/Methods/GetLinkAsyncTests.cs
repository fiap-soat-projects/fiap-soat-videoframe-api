...
public class GetLinkAsyncTests : VideoControllerDependenciesMock
{
    [Fact]
    public async Task When_Valid_Id_And_UserRequest_Then_Returns_VideoLinkPresenter()
    {
        ...
        var expectedLink = "https://presigned-url.example.com/video";

        _videoUseCaseMock.GetLinkAsync(id, userId, Arg.Any<CancellationToken>())
    ...
    public async Task When_Different_Id_And_UserId_Then_Returns_Correct_Link(string id, string userId, string expectedLink)
    {
        // Arrange
        var userRequest = new UserRequest(userId, "John Doe", "recipient@example.com");
        var cancellationToken = new CancellationToken();

        _videoUseCaseMock.GetLinkAsync(id, userId, cancellationToken).Returns(expectedLink);
        ...
        // Assert
        result.Should().NotBeNull();
        await _videoUseCaseMock.Received(1).GetLinkAsync(id, userId, cancellationToken);
    }
